		...
		$.getJSON('/getBackupStatus', function (data) {
			...
			for (i = 0, l = hp1.length; i < l; i++) {
				...
				if (date.getDate() == today.getDate()) {
					$('#t_erp_2_d td:eq(2)').html(hp1[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_2_d td:eq(1)').html(hp1[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_2_d td:eq(0)').html(hp1[i][1]);
				}				
			}
			for (i = 0, l = cp6.length; i < l; i++) {
				var date = new Date(cp6[i][0]);
				if (date.getDate() == today.getDate()) {
					$('#t_erp_2_d td:eq(5)').html(cp6[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_2_d td:eq(4)').html(cp6[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_2_d td:eq(3)').html(cp6[i][1]);
				}				
			}
			for (i = 0, l = cpf.length; i < l; i++) {
				var date = new Date(cpf[i][0]);
				if (date.getDate() == today.getDate()) {
					$('#t_erp_2_d td:eq(8)').html(cpf[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_2_d td:eq(7)').html(cpf[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_2_d td:eq(6)').html(cpf[i][1]);
				}				
			}
			for (i = 0, l = cpo.length; i < l; i++) {
				var date = new Date(cpo[i][0]);
				if (date.getDate() == today.getDate()) {
					$('#t_erp_2_d td:eq(11)').html(cpo[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_2_d td:eq(10)').html(cpo[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_2_d td:eq(9)').html(cpo[i][1]);
				}				
			}
			...
			for (i = 0, l = ep6.length; i < l; i++) {
				...
				if (date.getDate() == today.getDate()) {
					$('#t_erp_1_d td:eq(2)').html(ep6[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_1_d td:eq(1)').html(ep6[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_1_d td:eq(0)').html(ep6[i][1]);
				}
			}
			for (i = 0, l = ep7.length; i < l; i++) {
				var date = new Date(ep7[i][0]);
				if (date.getDate() == today.getDate()) {
					$('#t_erp_1_d td:eq(5)').html(ep7[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_1_d td:eq(4)').html(ep7[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_1_d td:eq(3)').html(ep7[i][1]);
				}
			}
			for (i = 0, l = cop.length; i < l; i++) {
				var date = new Date(cop[i][0]);
				if (date.getDate() == today.getDate()) {
					$('#t_erp_1_d td:eq(8)').html(cop[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_1_d td:eq(7)').html(cop[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_1_d td:eq(6)').html(cop[i][1]);
				}
			}
			for (i = 0, l = ep9.length; i < l; i++) {
				var date = new Date(ep9[i][0]);
				if (date.getDate() == today.getDate()) {
					$('#t_erp_1_d td:eq(11)').html(ep9[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_1_d td:eq(10)').html(ep9[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_1_d td:eq(9)').html(ep9[i][1]);
				}
			}
			for (i = 0, l = emp.length; i < l; i++) {
				var date = new Date(emp[i][0]);
				if (date.getDate() == today.getDate()) {
					$('#t_erp_1_d td:eq(14)').html(emp[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_1_d td:eq(13)').html(emp[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_1_d td:eq(12)').html(emp[i][1]);
				}
			}
			for (i = 0, l = ep1.length; i < l; i++) {
				var date = new Date(ep1[i][0]);
				if (date.getDate() == today.getDate()) {
					$('#t_erp_1_d td:eq(17)').html(ep1[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_1_d td:eq(16)').html(ep1[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_1_d td:eq(15)').html(ep1[i][1]);
				}
			}
			//纪检监察
			for (i = 0, l = jp1.length; i < l; i++) {
				...
				if (date.getDate() == today.getDate()) {
					$('#t_jp1_d td:eq(2)').html(jp1[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_jp1_d td:eq(1)').html(jp1[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_jp1_d td:eq(0)').html(jp1[i][1]);
				}
			}
			//BW on Oracle
			for (i = 0, l = bp9.length; i < l; i++) {
				...
				if (date.getDate() == today.getDate()) {
					$('#t_erp_1_d td:eq(20)').html(bp9[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_1_d td:eq(19)').html(bp9[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_1_d td:eq(18)').html(bp9[i][1]);
				}
			}
			for (i = 0, l = bpc.length; i < l; i++) {
				var date = new Date(bpc[i][0]);
				if (date.getDate() == today.getDate()) {
					$('#t_erp_1_d td:eq(23)').html(bpc[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_1_d td:eq(22)').html(bpc[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_1_d td:eq(21)').html(bpc[i][1]);
				}
			}
			for (i = 0, l = bpd.length; i < l; i++) {
				var date = new Date(bpd[i][0]);
				if (date.getDate() == today.getDate()) {
					$('#t_erp_1_d td:eq(26)').html(bpd[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_1_d td:eq(25)').html(bpd[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_1_d td:eq(24)').html(bpd[i][1]);
				}
			}
			//Deprecated ERP 1.0
			/*for (i = 0, l = wp1.length; i < l; i++) {
				...
				if (date.getDate() == today.getDate()) {
					$('#t_erp_1_d td:eq(29)').html(wp1[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_1_d td:eq(28)').html(wp1[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_1_d td:eq(27)').html(wp1[i][1]);
				}
			}
			for (i = 0, l = bp1.length; i < l; i++) {
				var date = new Date(bp1[i][0]);
				if (date.getDate() == today.getDate()) {
					$('#t_erp_1_d td:eq(32)').html(bp1[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_1_d td:eq(31)').html(bp1[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_1_d td:eq(30)').html(bp1[i][1]);
				}
			}
			for (i = 0, l = pp1.length; i < l; i++) {
				var date = new Date(pp1[i][0]);
				if (date.getDate() == today.getDate()) {
					$('#t_erp_1_d td:eq(35)').html(pp1[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_1_d td:eq(34)').html(pp1[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_1_d td:eq(33)').html(pp1[i][1]);
				}
			}
			for (i = 0, l = xp1.length; i < l; i++) {
				var date = new Date(xp1[i][0]);
				if (date.getDate() == today.getDate()) {
					$('#t_erp_1_d td:eq(38)').html(xp1[i][1]);
				}
				if (date.getDate() == ystd.getDate()) {
					$('#t_erp_1_d td:eq(37)').html(xp1[i][1]);
				}
				if (date.getDate() == dbyd.getDate()) {
					$('#t_erp_1_d td:eq(36)').html(xp1[i][1]);
				}
			}*/
